(module
  ...
  (func $valueHandType (param $handPointer i32)
    (result i32)
    (local $i i32)
    (local $j i32)
    (local $char i32)
    (local $count i32)
    (local $maxCount i32)
    (local $jokerValue i32)
    (local $jokerValueIndex i32)
    (local $comparisonValue i32)
    
    loop $lJokerValue
      local.get $handPointer
      local.get $jokerValueIndex
      i32.add
      i32.load8_u
      local.set $jokerValue
        ...
        if
          ...
          loop $lj
            ...
            if
              ...
              i32.add
              i32.load8_u
              local.tee $comparisonValue
              i32.const 0x4A ;; J
              i32.eq
              if
                local.get $jokerValue
                local.set $comparisonValue
              end
              
              local.get $comparisonValue
              local.get $char
              i32.eq
              if
                local.get $count
